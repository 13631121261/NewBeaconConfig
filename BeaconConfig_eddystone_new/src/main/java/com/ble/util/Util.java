              ...
              //  System.out.println("bbMAC="+result.getDevice().getAddress());
                ...
                if (adv != null && adv.length >= 5 && (adv[1]&0xff) == 0xFE && (adv[0]&0xff) == 0xAA) {
                    ...
                    if (beacon.getType() == 0) {
                        ...
                        String urldata="";
                        byte[] urls;
                        if(adv[adv.length-1]<14){
                            beacon.setUrl_tail(adv[adv.length-1]);
                            urldata= dd.substring(10, dd.length()-2);
                            urls = StringUtils.HexToByteArr(urldata);
                            beacon.setUrl_data(new String(urls));

                        }else{
                            beacon.setUrl_tail(14);
                             urldata = dd.substring(10, dd.length());
                             urls = StringUtils.HexToByteArr(urldata);
                            beacon.setUrl_data(new String(urls));
                        }
                        ...
                        if (beacon.getTlm_type() == 0) {
                            ...
                            beacon.setTlm_bt(((adv[4]&0xff) * 256) + (adv[5]&0xff));
...
//0x0201061716AAFE00ECF0C8F11200F4F11200F4F0C8F11200F40000020A00
// 12FF
// 4C4B
// 0BCF0A
// 00
...
// 08094B313230306634
        ...
        //    System.out.println(beacon.getAddress()+"d'd'd'd'd'd'd'd'd'd'd'd'd'd'd="+StringUtils.binary2(data[9]));
            ...
            else if(data!=null&&data.length==6&&data[1]==0x4B&&data[0]==0x4C){
            //電量
            int bt=((data[02]&0xff)*256)+(data[3]&0xff);
          //  System.out.println("用户数据"+(data[02]&0xff)*256);
